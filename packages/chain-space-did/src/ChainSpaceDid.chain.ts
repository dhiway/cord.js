...
export async function isChainSpaceStored(spaceUri: SpaceUri): Promise<boolean> {
  try {
    ...
    const encoded = await api.query.chainSpaceDid.spaces(identifier)
...
export async function isAuthorizationStored(
  ...
  try {
    ...
    const encoded = await api.query.chainSpaceDid.authorizations(identifier)
...
export async function prepareCreateSpaceExtrinsic(
  ...
  try {
    ...
    const tx = api.tx.chainSpaceDid.create(chainSpace.digest)
...
export async function prepareCreateSubSpaceExtrinsic(
  ...
  try {
    ...
    const tx = api.tx.chainSpaceDid.subspaceCreate(chainSpace.digest, count, parent?.replace('space:cord:', ''))
    const extrinsic = await Did.authorizeTx(
      creatorUri,
      tx,
      signCallback,
...
export async function sudoApproveChainSpace(
  ...
  try {
    ...
    const tx = api.tx.chainSpaceDid.approve(spaceId, capacity)
...
function dispatchDelegateAuthorizationTx(
  ...
  switch (permission) {
    case Permission.ASSERT:
      return api.tx.chainSpaceDid.addDelegate(spaceId, delegateId, authId)
    case Permission.DELEGATE:
      return api.tx.chainSpaceDid.addDelegator(spaceId, delegateId, authId)
    case Permission.ADMIN:
      return api.tx.chainSpaceDid.addAdminDelegate(spaceId, delegateId, authId)
    default:
...
export async function fetchFromChain(
  ...
  try {
    ...
    const spaceEntry = await api.query.chainSpaceDid.spaces(spaceId)
...
export async function fetchAuthorizationFromChain(
  ...
  try {
    ...
    const authEntry = await api.query.chainSpaceDid.authorizations(authId)
...
export async function prepareUpdateTxCapacityExtrinsic(
  ...
  try {
    ...
    const tx = api.tx.chainSpaceDid.updateTransactionCapacitySub(spaceUri.replace('space:cord:', ''), new_capacity)    
    const extrinsic = await Did.authorizeTx(
      creatorUri,
      tx,
      signCallback,
