...
import type {
  ...
  AccountId,
  H256,
  ISpaceDetailsAccountType,
  Option,
  ISpaceAuthorizationAccountType,
  PermissionType,
...
export async function prepareCreateSpaceExtrinsic(
  ...
  try {
    ...
    const extrinsic = api.tx.chainSpace.create(chainSpace.digest)
    return extrinsic;
 ...
 *
 * try {
 *   const result = await dispatchToChain(chainSpace, creatorUri, authorAccount);
...
export async function dispatchToChain(
  ...
  try {
    ...
    if (!exists) {
        const extrinsic = await prepareCreateSpaceExtrinsic(chainSpace)
...
export async function prepareCreateSubSpaceExtrinsic(
  ...
  try {
    const api = ConfigService.get('api')
    const extrinsic = api.tx.chainSpace.subspaceCreate(chainSpace.digest, count, parent?.replace('space:cord:', ''))
...
export async function getUriForAuthorization(
  ...
  const scaleEncodedSpaceId = api
    ...
    .toU8a()
  const scaleEncodedAuthDelegate = api
    .createType<AccountId>('AccountId', delegatorAddress)
    .toU8a()
 ...
 * @param delegateAddress - Address of the delegate being authorized.
...
export async function prepareDelegateAuthorizationExtrinsic(
  ...
  switch (permission) {
    case Permission.ASSERT:
      return api.tx.chainSpace.addDelegate(spaceId, delegateAddress, authId)
    case Permission.DELEGATE:
      return api.tx.chainSpace.addDelegator(spaceId, delegateAddress, authId)
    case Permission.ADMIN:
      return api.tx.chainSpace.addAdminDelegate(spaceId, delegateAddress, authId)
    default:
...
export async function dispatchDelegateAuthorization(
  request: ISpaceAuthorizationAccountType,
  authorAccount: CordKeyringPair,
  authorizationUri: AuthorizationUri,
 ...
 *          creator's address, transaction capacity, transaction usage, approval status, and archival status.
...
function decodeSpaceDetailsfromChain(
  ...
  const decodedDetails: ISpaceDetailsAccountType = {
    uri: spaceUri,
    creatorAddress: chainStatement.creator.toString(),
 ...
 *          the space URI, creator address, transaction capacity, and other relevant data.
 *
...
export async function fetchFromChain(
  spaceUri: SpaceUri
): Promise<ISpaceDetailsAccountType | null> {
  try {
    const api = ConfigService.get('api')
    const spaceId = uriToIdentifier(spaceUri)
...
function decodeAuthorizationDetailsfromChain(
  ...
  const decodedDetails: ISpaceAuthorizationAccountType = {
    uri: identifierToUri(
      DecoderUtils.hexToString(chainAuth.spaceId.toString())
    ) as SpaceUri,
    delegateAddress: chainAuth.delegate.toString(),
    permission: authorizationPermissionsFromChain(chainAuth.permissions),
    authorizationUri,
    creatorAddress: chainAuth.delegate.toString(),
  }
  return decodedDetails
}
...
export async function fetchAuthorizationFromChain(
  ...
  try {
    ...
    const authDetails: ISpaceAuthorizationAccountType =
...
export async function prepareUpdateTxCapacityExtrinsic(
  ...
  try {
    ...
    const extrinsic = api.tx.chainSpace.updateTransactionCapacitySub(spaceUri.replace('space:cord:', ''), new_capacity)    
    return extrinsic;
...
export async function dispatchUpdateTxCapacityToChain(
  ...
  try {
    const extrinsic = await prepareUpdateTxCapacityExtrinsic(space, new_capacity)
    await Chain.signAndSubmitTx(extrinsic, authorAccount)
