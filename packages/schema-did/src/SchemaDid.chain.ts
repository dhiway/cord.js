...
import type { PalletSchemaDidSchemaEntry } from '@cord.network/augment-api'
import {
  SchemaDigest,
  DidUri,
  ISchema,
...
import { encodeCborSchema, verifyDataStructure } from './SchemaDid.js'
...
export async function isSchemaStored(schema: ISchema): Promise<boolean> {
  ...
  const encoded = await api.query.schemaDid.schemas(identifier)
...
export async function dispatchToChain(
  ...
  try {
    ...
    const tx = api.tx.schemaDid.create(encodedSchema, authorizationId)
 ...
 */
function fromChain(
  encodedEntry: Option<PalletSchemaDidSchemaEntry>,
  schemaUri: ISchema['$id']
): ISchemaDetails | null {
...
export async function fetchFromChain(
  ...
  try {
    ...
    const schemaEntry = await api.query.schemaDid.schemas(cordSchemaId)
