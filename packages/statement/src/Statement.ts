 ...
 * @internal
 */
export function verifyDataStructure(input: IStatementEntryAccountType): void {
  if (!input.digest) {
 ...
 * @param creatorAddress - The address of the statement's creator.
...
export function buildFromProperties(
  ...
  const stmtUri = getUriForStatement(digest, spaceUri, creatorAddress)

  const statement: IStatementEntryAccountType = {
    elementUri: stmtUri,
    digest,
    creatorAddress,
    spaceUri,
 ...
 */
export function isIStatement(input: unknown): input is IStatementEntryAccountType {
  try {
    verifyDataStructure(input as IStatementEntryAccountType)
  } catch (error) {
    return false
  }
  return true
 ...
 * @param digest - The hexadecimal string representing the digest of the statement.
 * @param creator - (Optional) The account address of the statement's creator for verification.
  ...
  try {
    ...
    if (creator) {
      if (creator !== statementStatus.creatorAddress) {
        console.log("creator", creator, "statementStatus.creator", statementStatus.creatorAddress)
        return {
          isValid: false,
