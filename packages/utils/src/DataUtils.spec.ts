...
it('validates address with prefix 29', () => {
  expect(() => validateAddress(encodeAddress(key, 29), 'test')).not.toThrow()
  expect(validateAddress(encodeAddress(key, 29), 'test')).toBe(true)
})
