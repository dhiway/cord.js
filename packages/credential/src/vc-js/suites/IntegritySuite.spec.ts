...
beforeAll(async () => {
  ...
  credential.proof.some((p) => {
    if (p.type === CORD_CREDENTIAL_DIGEST_PROOF_TYPE) {
...
describe('jsigs', () => {
  ...
  describe('verification', () => {
    ...
    it('verifies Cord Credential Digest Proof', async () => {
      await expect(
        jsigs.verify(credential, { suite, purpose, documentLoader })
      ).resolves.toMatchObject({ verified: true })
    })

    it('verifies Cord Credential Digest Proof with props removed', async () => {
